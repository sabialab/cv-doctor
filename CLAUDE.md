...
| 诊断（**当前实现**） | `stub_pipeline` | 桩数据，非真实 DOCX/LLM；用于打通 UI/API/测试 |
...
uv run ruff check src/api src/main.py src/p0_models.py \
  src/services/session_store.py src/services/stub_pipeline.py tests/test_api.py
uv run pytest -q
