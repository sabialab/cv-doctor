...
| 风险 | 等级 | 对策 |
|------|------|------|
| 免费AI替代（最大风险） | **高** | 不卖"AI改写"，卖"可审计修改+风险提示+DOCX导出" |
| 竞品跟进 | **高** | 不和模板工具正面打；聚焦"JD→diff手术单"；做实证据链 |
| LLM 编造经历 | **高** | Fact Ledger + Policy Guard + evidence-based rewrite + diff |
| PDF/DOCX 解析质量 | **中** | DOCX优先、粘贴文本降级、解析后用户确认 |
